"""
Binary tree engine v2 — tracking center aware, BV allocation support.
"""
...
async def place_triple_header(db, distributor_id, sponsor_position_id, preferred_leg=None):
    ...
    c2_pos = TreePosition(parent_id=c1_pos.id, leg="left", depth=c1_pos.depth + 1,
                          path=f"{c1_pos.path or str(c1_pos.id)}.{c1_pos.id}")
    ...
    c3_pos = TreePosition(parent_id=c1_pos.id, leg="right", depth=c1_pos.depth + 1,
                          path=f"{c1_pos.path or str(c1_pos.id)}.{c1_pos.id}")
...
async def _bfs_place(db, start_position_id, preferred_leg=None):
    if preferred_leg in ("left", "right"):
        taken = (await db.execute(
            select(TreePosition).where(TreePosition.parent_id == start_position_id, TreePosition.leg == preferred_leg)
        )).scalar_one_or_none()
        if not taken:
            parent = await db.get(TreePosition, start_position_id)
            pos = TreePosition(
                parent_id=start_position_id, leg=preferred_leg,
                depth=(parent.depth + 1) if parent else 1,
                path=f"{parent.path}.{start_position_id}" if parent and parent.path else str(start_position_id),
            )
    ...
    while queue:
        ...
        for leg in ("left", "right"):
            if leg not in taken_legs:
                ...
                pos = TreePosition(
                    parent_id=cur, leg=leg,
                    depth=(parent.depth + 1) if parent else 1,
                    path=f"{parent.path}.{cur}" if parent and parent.path else str(cur),
                )
                db.add(pos); await db.flush(); return pos
        for child in children: queue.append(child.id)
    raise RuntimeError(f"No open slot from position {start_position_id}")
...
async def _subtree_cv(db, root_pos_id, cycle_id):
    """
    BV allocation aware:
    - Orders with center_id set → count only if that center is in this subtree
    - Orders without center_id → count if the distributor has exactly 1 center (single-center BA)
    """
    center_ids = await _subtree_center_ids(db, root_pos_id)
    if not center_ids:
        return 0

    # CV from explicitly allocated orders
    r1 = await db.execute(
        select(func.sum(Order.cv_total)).where(
            Order.center_id.in_(center_ids),
            Order.cycle_id == cycle_id,
            Order.status == OrderStatus.VERIFIED,
            Order.order_type == OrderType.BA_PURCHASE,
        )
    )
    cv = r1.scalar_one_or_none() or 0

    # CV from unallocated orders — only for single-center BAs (avoids double-count)
    all_dist_ids = list({
        r[0] for r in (await db.execute(
            select(TrackingCenter.distributor_id).where(TrackingCenter.id.in_(center_ids))
        )).all()
    })
    if all_dist_ids:
        multi_center_dists = {
            r[0] for r in (await db.execute(
                select(TrackingCenter.distributor_id)
                .where(TrackingCenter.distributor_id.in_(all_dist_ids))
                .group_by(TrackingCenter.distributor_id)
                .having(func.count(TrackingCenter.id) > 1)
            )).all()
        }
        single_center_dists = [d for d in all_dist_ids if d not in multi_center_dists]
        if single_center_dists:
            r2 = await db.execute(
                select(func.sum(Order.cv_total)).where(
                    Order.distributor_id.in_(single_center_dists),
                    Order.center_id.is_(None),
                    Order.cycle_id == cycle_id,
                    Order.status == OrderStatus.VERIFIED,
                    Order.order_type == OrderType.BA_PURCHASE,
                )
            )
            cv += r2.scalar_one_or_none() or 0
