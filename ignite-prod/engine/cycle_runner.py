"""
Cycle Runner — idempotent, center-keyed, with detailed error logging.
"""
import json, logging
...
async def run_cycle_close(cycle_id: int, actor_id: int, session_factory: async_sessionmaker):
    """Entry point called from BackgroundTasks. Uses its own DB session."""
    async with session_factory() as db:
        try:
            await _do_close(db, cycle_id, actor_id)
            await db.commit()
            logger.info(f"Cycle {cycle_id} closed successfully")
        except Exception as exc:
            await db.rollback()
            logger.exception(f"Cycle {cycle_id} failed: {exc}")
            # Write error event + reset cycle to OPEN so it can be retried
            async with session_factory() as db2:
                try:
                    cycle = await db2.get(Cycle, cycle_id)
                    if cycle and cycle.status not in (CycleStatus.CLOSED, CycleStatus.APPROVED):
                        cycle.status = CycleStatus.OPEN
                    db2.add(SystemEvent(
                        event_type="cycle_close_failed",
                        payload=json.dumps({"cycle_id": cycle_id, "error": str(exc)}),
                        status="error",
                    ))
                    await db2.commit()
                except Exception as e2:
                    logger.exception(f"Failed to write error event: {e2}")
...
async def _do_close(db: AsyncSession, cycle_id: int, actor_id: int):
    ...
    if cycle.status == CycleStatus.CLOSED:
        raise ValueError(f"Cycle {cycle_id} is already closed")
    if cycle.status == CycleStatus.APPROVED:
        raise ValueError(f"Cycle {cycle_id} is already approved")

    # ── Load plan config ──────────────────────────────────────────────────
    plan_cfg = await db.get(PlanConfig, 1)
    if not plan_cfg:
        raise ValueError("No plan config found (id=1). Run seed first.")

    plan = PlanParams(
        cv_per_step=plan_cfg.cv_per_step,
        flush_ratio=float(plan_cfg.flush_ratio),
        tds_rate=float(plan_cfg.tds_rate),
        tds_threshold_inr=plan_cfg.tds_threshold_inr,
        inr_rounding=plan_cfg.inr_rounding,
        coin_lifetime_cap=plan_cfg.coin_lifetime_cap,
    ...
    if not rank_cfgs:
        raise ValueError("No rank config found. Run seed first.")

    rank_table = [
        RankRow(r.rank_name, r.min_cumulative_steps, r.min_direct_cv,
                r.step_rate_inr, r.max_weekly_steps, r.matching_bonus_levels,
                r.maintenance_bonus_inr, r.maintenance_hold_months)
        for r in rank_cfgs
    ]

    # ── Load active centers ───────────────────────────────────────────────
    centers = (await db.execute(
        select(TrackingCenter)
        .join(Distributor, TrackingCenter.distributor_id == Distributor.id)
        .where(
    ...
    logger.info(f"Cycle {cycle_id}: processing {len(centers)} active centers")

    if len(centers) == 0:
        # No active BAs — still mark cycle closed so it's not stuck
        cycle.status = CycleStatus.CLOSED
        cycle.closed_at = datetime.now(tz=timezone.utc)
        db.add(SystemEvent(
            event_type="cycle_closed",
            payload=json.dumps({"cycle_id": cycle_id, "centers": 0, "note": "No active centers"}),
            status="info",
        ))
        return
    ...
    # ── Pass 1: step commissions (needed for matching bonus map) ──────────
    ...
    for center in centers:
        try:
            ...
            logger.warning(f"Pass1 failed for center {center.id}: {e}")

    # ── Pass 2: finalize + write records ─────────────────────────────────
    ...
    for center in centers:
        ...
        if (await db.execute(
            select(CommissionLedger).where(
                CommissionLedger.center_id == center.id,
                CommissionLedger.cycle_id == cycle_id,
            )
        )).scalar_one_or_none():
            continue
        ...
        try:
            ...
            if center.position_id and center.id in pass1:
                ...
                matching = await calculate_matching_bonus(
                    db, center.position_id, cycle_id, rank_name, rank_table, step_comm_map
                )

            # FY gross before this cycle
            if center.distributor_id not in fy_gross_map:
                tax = (await db.execute(
                    select(TaxTracking).where(
                        TaxTracking.distributor_id == center.distributor_id,
                        TaxTracking.financial_year == fy,
                    )
                )).scalar_one_or_none()
                fy_gross_map[center.distributor_id] = tax.gross_income_inr if tax else 0

            final = await _calc_center(
                db, center, cycle_id, plan, rank_table, fy,
            ...
            # Write CommissionLedger
            db.add(CommissionLedger(
                center_id=center.id,
                distributor_id=center.distributor_id,
                cycle_id=cycle_id,
                rank_at_cycle=final.rank_name,
                steps_earned=final.steps_earned,
            ...
            db.add(DistributorCarryForward(
                ...
                distributor_id=center.distributor_id,
                cycle_id=cycle_id,
                left_cv_carry=final.left_cv_carry_out,
                right_cv_carry=final.right_cv_carry_out,
            ))

            # Accumulate BA gross
            ba_gross[center.distributor_id] = (
                ba_gross.get(center.distributor_id, 0) + final.gross_commission_inr
            )
            fy_gross_map[center.distributor_id] += final.gross_commission_inr

            # Update rank history
            hist = (await db.execute(
                select(DistributorRank)
                .where(DistributorRank.distributor_id == center.distributor_id)
                .order_by(DistributorRank.achieved_at.desc())
                .limit(1)
            )).scalars().all()
            if not hist or hist[0].rank_name != final.rank_name:
                db.add(DistributorRank(
                    distributor_id=center.distributor_id,
                    rank_name=final.rank_name,
                    achieved_at=cycle.end_date,
                    cycle_id=cycle_id,
            ...
            if coins:
                if final.green_coins_converted > 0:
                    ...
                    db.add(CoinTransaction(
                        distributor_id=center.distributor_id, center_id=center.id,
                        cycle_id=cycle_id, coin_type="green",
                        delta=-final.green_coins_converted,
                        reason="bridge_conversion",
                ...
                if final.yellow_coins_earned > 0:
                    ...
                    db.add(CoinTransaction(
                        ...
                        delta=final.yellow_coins_earned,
            ...
            logger.warning(f"Pass2 failed for center {center.id}: {e}")
            continue

    # ── TDS tracking — one update per BA ──────────────────────────────────
    for dist_id, gross in ba_gross.items():
        if gross <= 0:
            continue
        tds_cycle = (await db.execute(
            select(func.sum(CommissionLedger.tds_deducted_inr)).where(
                CommissionLedger.distributor_id == dist_id,
                CommissionLedger.cycle_id == cycle_id,
            )
        ...
        tax = (await db.execute(
            select(TaxTracking).where(
                TaxTracking.distributor_id == dist_id,
                TaxTracking.financial_year == fy,
            )
        )).scalar_one_or_none()
        if tax:
            ...
            db.add(TaxTracking(
                distributor_id=dist_id, financial_year=fy,
                gross_income_inr=gross, tds_deducted_inr=tds_cycle,
            ))

        ba = await db.get(Distributor, dist_id)
        stmt = (await db.execute(
            select(TdsStatement).where(
                TdsStatement.distributor_id == dist_id,
                TdsStatement.financial_year == fy,
            )
        )).scalar_one_or_none()
        if stmt:
    ...
    totals = (await db.execute(
        select(
            func.sum(CommissionLedger.gross_commission_inr),
    ...
    db.add(SystemEvent(
        ...
        payload=json.dumps({
            "cycle_id": cycle_id,
            "centers": len(centers),
            "payout_inr": int(cycle.total_payout_inr),
            "tds_inr": int(cycle.total_tds_inr),
        }),
        status="info",
    ))
    logger.info(f"Cycle {cycle_id} closed: {len(centers)} centers, payout ₹{cycle.total_payout_inr}")


async def _calc_center(db, center, cycle_id, plan, rank_table, fy,
                       matching_bonus=0, fy_gross_before=None):
    if not center.position_id:
        return None
    ...
    carry = (await db.execute(
        ...
        .order_by(DistributorCarryForward.cycle_id.desc())
        .limit(1)
    )).scalar_one_or_none()
    carry_in = (
        (carry.left_cv_carry, carry.right_cv_carry)
        if carry and carry.cycle_id < cycle_id
        else (0, 0)
    )

    # Direct CV — use enum comparison
    ...
    inp = CycleInput(
        distributor_id=center.distributor_id,
        left_cv_this_cycle=left_cv,
        right_cv_this_cycle=right_cv,
        left_cv_carry_in=carry_in[0],
        right_cv_carry_in=carry_in[1],
        lifetime_steps_before=lifetime_steps,
        direct_cv_this_cycle=direct_cv,
        ...
        maintenance_months_at_rank=await _consec_months(db, center.distributor_id),
        maintenance_already_paid=await _maint_paid(db, center.distributor_id),
    )
...
async def _consec_months(db, distributor_id):
    rows = (await db.execute(
        select(DistributorRank)
        .where(DistributorRank.distributor_id == distributor_id)
        .order_by(DistributorRank.achieved_at.desc())
        .limit(12)
    )).scalars().all()
    if not rows:
        return 0
    current = rows[0].rank_name
    count = 0
    for r in rows:
        if r.rank_name == current:
            count += 1
        else:
            break
    return count
...
async def _maint_paid(db, distributor_id):
    return bool((await db.execute(
        ...
        .where(
            CommissionLedger.distributor_id == distributor_id,
            CommissionLedger.maintenance_bonus_inr > 0,
        )
        .limit(1)
    )).scalar_one_or_none())
