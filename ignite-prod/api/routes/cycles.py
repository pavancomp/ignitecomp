...
from db.models import Cycle, CycleStatus, CommissionLedger, TrackingCenter, AdminUser
...
async def create_cycle(body: CycleCreate, request: Request,
    ...
    if (await db.execute(select(Cycle).where(Cycle.cycle_code == body.cycle_code))).scalar_one_or_none():
    ...
    cycle = Cycle(cycle_code=body.cycle_code, cycle_type=body.cycle_type,
                  start_date=body.start_date, end_date=body.end_date, status=CycleStatus.OPEN)
...
async def close_cycle(cycle_id: int, background_tasks: BackgroundTasks,
                      request: Request, db: AsyncSession = Depends(get_db),
                      current_user: AdminUser = Depends(require_admin)):
    cycle = await db.get(Cycle, cycle_id)
    if not cycle: raise HTTPException(status_code=404, detail="Cycle not found")
...
async def cycle_commissions(cycle_id: int,
    ...
    # Enrich with center_number via join
    items = []
    for r in rows:
